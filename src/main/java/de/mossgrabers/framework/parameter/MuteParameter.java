...
/**
 * A parameter implementation for muting the selected track.
 *
 * @author Jürgen Moßgraber
 */
public class MuteParameter extends AbstractParameterImpl
{
     ...
     */
    public MuteParameter (final IModel model)
    {
    ...
    public int getValue ()
    {
        ...
        return selectedTrack.isPresent () && selectedTrack.get ().isMute () ? this.valueChanger.getUpperBound () - 1 : 0;
    }
    ...
    public void setValue (final IValueChanger valueChanger, final int value)
    {
        ...
        if (selectedTrack.isPresent ())
            selectedTrack.get ().setMute (value > 0);
    }
    ...
    public void changeValue (final IValueChanger valueChanger, final int value)
    {
        final Optional<ITrack> selectedTrack = this.model.getTrackBank ().getSelectedItem ();
        if (selectedTrack.isPresent ())
            selectedTrack.get ().setMute (valueChanger.isIncrease (value));
    }
    ...
    public void inc (final double increment)
    {
        final Optional<ITrack> selectedTrack = this.model.getTrackBank ().getSelectedItem ();
        if (selectedTrack.isPresent ())
            selectedTrack.get ().setMute (increment > 0);
    }
    ...
    {
        final Optional<ITrack> selectedTrack = this.model.getTrackBank ().getSelectedItem ();
        return selectedTrack.isPresent () && selectedTrack.get ().isMute () ? "Muted" : "-";
    }
